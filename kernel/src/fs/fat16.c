...
void fat_list_init(fat_list *list) {
    list->head = NULL;
    list->size = 0;
}

fat_node *fat_list_push_back(fat_list *list, const fat *data) {
...
fat_node *fat_list_find_vol_id(const fat_list *list, uint32_t vol_id) {
...
void fat_list_free(fat_list *list) {
    fat_node *cur = list->head;
    while (cur) {
        fat_node *tmp = cur->next;
        kfree(cur);
        cur = tmp;
    }
    list->head = NULL;
...
fat *read_fat(enum StorageDevType type, uint8_t controller, uint8_t port) {
    ...
    if (type == AHCI) {
        if (ahci_read(controller, port, FAT16_LBA, 1, sector) != 0) {
            frame_free(phys);
            return NULL;
        }
    } else {
...
uint8_t fat16_read(enum StorageDevType type, uint8_t controller, uint8_t port, uint32_t lba, uint8_t count, void *buffer) {
    if (type != AHCI)
        return 1;
    return ahci_read(controller, port, lba, count, buffer);
}
...
uint8_t fat16_format(enum StorageDevType type, uint8_t controller, uint8_t port, uint32_t total_sectors) {
    ...
    if (ahci_write(controller, port, FAT16_LBA, 1, sector) != 0) {
        frame_free(phys);
        return 4;
    }
    ...
    for (uint8_t i = 0; i < FAT16_NUM_FATS; i++) {
        ...
        if (ahci_write(controller, port, base, 1, sector) != 0) {
            frame_free(phys);
            return 5;
        }
        memset(sector, 0, FAT16_SECTOR_SIZE);
        for (uint16_t j = 1; j < fat_size; j++) {
            if (ahci_write(controller, port, base + j, 1, sector) != 0) {
                frame_free(phys);
                return 6;
            }
        }
    ...
    for (uint16_t i = 0; i < root_dir_sectors; i++) {
        if (ahci_write(controller, port, root_base + i, 1, sector) != 0) {
            frame_free(phys);
            return 7;
        }
    }
...
uint8_t fat16_init(enum StorageDevType type, uint8_t controller, uint8_t port) {
    ...
    fat *f = read_fat(type, controller, port);
    if (!f) return 1;
    ...
    if (!fat_list_push_back(&fat16_volumes, f)) {
        kfree(f);
        return 4;
    }
