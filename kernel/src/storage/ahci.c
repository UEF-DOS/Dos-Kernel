...
static uint64_t pci_read_bar64(uint8_t bus, uint8_t slot, uint8_t func, uint8_t offset) {
    uint32_t low = pci_read_config_dword(bus, slot, func, offset);
    if (low & 1)
        return 0;

    uint64_t addr_low  = (uint64_t)(low & 0xfffffff0u);
    uint64_t addr_high = (uint64_t)pci_read_config_dword(bus, slot, func, offset + 4);
...
static uint8_t pci_is_ahci_device(uint8_t bus, uint8_t slot, uint8_t func) {
    ...
    uint8_t prog_if    = pci_read_config_byte(bus, slot, func, 0x09);
    ...
    for (uint16_t bus = 0; bus < 256; bus++) {
        for (uint8_t slot = 0; slot < 32; slot++) {
            ...
            for (uint8_t func = 0; func < func_limit; func++) {
                uint16_t vendor = pci_read_config_word(bus, slot, func, 0);
                if (vendor == 0xffff)
                    continue;
...
const ahci_controller_t *ahci_get_controller(uint8_t index) {
