...
void map_page(uint64_t *pml4, uint64_t virt_addr, void *phys_addr, uint64_t flags) {
    ...
    if (!(pml4[pml4_index] & 1)) {
        ...
        pml4[pml4_index] = (uint64_t)phys | TABLE_FLAGS;
    }
    ...
    if (!(pdpt[pdpt_index] & 1)) {
        ...
        pdpt[pdpt_index] = (uint64_t)phys | TABLE_FLAGS;
    }
    ...
    if (!(pd[pd_index] & 1)) {
        ...
        pd[pd_index] = (uint64_t)phys | TABLE_FLAGS;
    }

    uint64_t *pt = (uint64_t *)phys_to_virt(ENTRY_PHYS(pd[pd_index]));

    pt[pt_index] = ENTRY_PHYS((uint64_t)phys_addr) | flags | 0x1;
...
void vmm_map_range(uint64_t *pml4, uint64_t virt, void *phys, uint64_t size, uint64_t flags) {
    ...
    for (uint64_t offset = 0; offset < size; offset += 4096)
        map_page(pml4, virt + offset, (uint8_t *)phys + offset, flags);
}
...
void *vmm_get_phys(uint64_t *pml4, uint64_t virt) {
    ...
    if (!(pml4[pml4_index] & 1)) return NULL;
    uint64_t *pdpt = (uint64_t *)phys_to_virt(ENTRY_PHYS(pml4[pml4_index]));

    if (!(pdpt[pdpt_index] & 1)) return NULL;
    uint64_t *pd = (uint64_t *)phys_to_virt(ENTRY_PHYS(pdpt[pdpt_index]));

    if (!(pd[pd_index] & 1)) return NULL;
    uint64_t *pt = (uint64_t *)phys_to_virt(ENTRY_PHYS(pd[pd_index]));

    if (!(pt[pt_index] & 1)) return NULL;

    void *phys = (void *)(ENTRY_PHYS(pt[pt_index]) + (virt & 0xFFF));
